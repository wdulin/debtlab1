...
function DebtLabBasic() {
	 ...
	 */
	this.DEFAULT_BEGINNING_LENDER_BALANCE = 1000;
	/**
...
//        console.log("fromDeposits:", this.notesOutstanding[i].fromDeposits);
        ...
        if(this.notesOutstanding[i].due <= this.currentDayNumber ) {
            ...
            deposits = this.notesOutstanding[i].fromDeposits;
            if(this.defaultOnPayBackFlag) {
                this.debtToLender -= (principal + interest);
                this.notesDefaultedQueue += 1;
            } else {
                
                // if loan was from deposits then
                // payback principal to deposits
                // and interest to lender
                if(deposits) {
...
/**
 * Do a one time borrow from lender deposits
 * 
 * @method
 */
DebtLabBasic.prototype.doLendFromDeposits = function() {
...
DebtLabBasic.prototype.getLenderDepositsBalance = function() {
    return this.lenderDepositsBalance;
}
 ...
 */
DebtLabBasic.prototype.doBorrow = function(fromDeps) {
    
    // Check for one time lend from deposits
    var deposits = false;
    if(!fromDeps) {
        deposits = this.autoLendFromLenderDepositsFlag;
    } else {
        // console.log("Had deposits Argument:" + fromDeposits);
        deposits = fromDeps;
    }
    ...
    this.notesOutstanding.push({
        ...
        interest: interest,
        fromDeposits: deposits
        
    });
    
    if (deposits) {
        this.lenderDepositsBalance -= this.noteAmount;
        console.log("From depostis");
    } else {
        this.lenderAccountBalance -= this.noteAmount;
        console.log("From Lender");
    }
...
DebtLabBasic.prototype.handleAutoFlags = function() {
    ...
    // Automatically replenish Deposits
    if (this.lenderDepositsBalance < (this.currentMoneySupply / 2)) {
        this.lenderDepositsBalance = (this.currentMoneySupply / 2);
    }
